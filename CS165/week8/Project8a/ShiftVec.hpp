...
{
private:
	std::vector<T> vect; //generic vector

public:
	ShiftVec(std::vector<T>); //constructor

	std::vector<T> getVec() const { return vect; } //getter

	void shiftRight(int steps); //shifts vector elements to the right
	void shiftLeft(int steps); //shifts vector elements to the left
};
