...
** Author: Conrad Lewin
** Date: 2/23/2016
** Description: Defines several functions that make up the functional capabilities of a to do list
** that writes to and loads data from a file
***************************************************************************************************/
...
{
	//cast the void* parameters to struct Task
	struct Task* newLeft = (struct Task*)left;
	struct Task* newRight = (struct Task*)right;

	//compare the priority value of the given value
...
TaskP createTask (int priority, char *desc)
{
	...
	//set that task's priority and description
