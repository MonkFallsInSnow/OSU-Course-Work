...
{
	assert(g != NULL);
	cirListDeque* stack = malloc(sizeof (cirListDeque)); //create a deque to be used as a stack

	clearVisited(g); //reset the visited property of each vertex
	initCirListDeque(stack); //initialize the deque
	addFrontCirListDeque(stack, source); //add the source node to the stack
	
	while (!isEmptyCirListDeque(stack)) //as long as there are verticies in the deque
	{
		//pop a vertex off the stack and mark it as visited
		Vertex* curVert = frontCirListDeque(stack);
		curVert->isVisited = 1;
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	assert(g != NULL);
	cirListDeque* queue = malloc(sizeof(cirListDeque)); //create a deque to be used as a queue

	clearVisited(g); //reset the visited property of each vertex
	initCirListDeque(queue); //initialize the deque
	addBackCirListDeque(queue, source); //add the source node to the

	while (!isEmptyCirListDeque(queue))  //as long as there are verticies in the deque
	{
		//remove a vertex from the front of the queue and mark it as visited
		Vertex* curVert = frontCirListDeque(queue);
		curVert->isVisited = 1;
		...
		for (int i = 0; i < curVert->numNeighbors; i++)
		{
			...
			if (!curVert->neighbors[i]->isVisited)
			{
				addBackCirListDeque(queue, curVert->neighbors[i]);
			}
		}
