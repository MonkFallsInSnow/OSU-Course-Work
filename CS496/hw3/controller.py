...
VALID_GENDERS = ['male','female','transgender','none']
VALID_USER_KEYS = ['account','user','fname','lname','email','username','gender','age']
VALID_DREAM_KEYS = ['account','user','dream','summary','mood','title','desc','tags','type','date']
#VALID_DREAM_KEYS = ['account','user','dream','summary','mood','title','desc','tags','type']
VALID_DREAM_TYPES = ['Epic Dream','Lucid Dream','Nightmare','Recurring Dream','False Awakening Dream','Procognitive Dream','None']
...
def addUser(payload):
	...
	if _verifyChoices(VALID_GENDERS,payload['gender']):
		return {'message':'Please use one of the following gender values -> {}'.format(VALID_GENDERS)},400
	else:
		try:	
			acc = ent.createAccount()
			...
			accKey.delete()
			return {'message' : 'Invalid value was passed in request body. Check the value of key -> age'},400

		return {'message':'User was created and added to the database', 'account':accKey.id(),'user':userKey.id()},201
...
def updateUser(payload):
	...
	if user != None:
		...
		if 'age' in payload:
			if payload['age'].isdigit():
				user.age = int(payload['age'])
			else:
				return {'message' : 'Invalid key value -> age must be an integer'},400
...
def getDreams(cls,kind1,kind2,id1,id2):
	results = cls.query(ancestor=ndb.Key(kind1,id1,kind2,id2)).fetch()
...
def addDream(payload):
	...
	try:
		ancestorPath = ndb.Key(ent.Account,int(payload['account']),ent.User,int(payload['user']))
...
def updateDream(payload):
	...
	if _verifyRequestKeys(VALID_DREAM_KEYS,payload):
		return {'message':'Please include the following keys in your request -> {}'.format(VALID_DREAM_KEYS)},400
		
	dream = ndb.Key(ent.Account,int(payload['account']),ent.User,int(payload['user']),ent.Dream,int(payload['dream'])).get()
	...
	if dream != None:
		...
		dreamKey = ent.saveEntity(dream)
		return {'message':'Dream resource has been updated','dream':dreamKey.id()},200
	else:
		return {'message':'Resource not found'},404
...
def deleteDream(userId,accId,dreamId):
	...
	dream = ndb.Key(ent.Account,int(accId),ent.User,int(userId),ent.Dream,int(dreamId)).get()

	if dream != None:
		dream.key.delete()
		return {'message':'Dream resource deleted'},200
	else:
