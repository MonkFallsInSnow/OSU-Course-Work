...
@app.route('/dream-catcher/v1/accounts/<int:accId>/users/<int:userId>/dreams',methods=['GET'])
def returnDreamByUser(userId,accId):
	'''gets dream by user key'''

	if request.method == 'GET':
		res,status = cntrl.getDreams(ent.Dream,ent.Account,ent.User,int(accId),int(userId))
		res = jsonify(res)
		res.headers['Status'] = status
	else:
		abort(405)
...
@app.route('/dream-catcher/v1/accounts/<int:accId>/users/<int:userId>/dreams/<int:dreamId>',methods=['DELETE'])
